...
import { generateStaticParams } from '@/app/(localized-pages)/[lang]/[country]/[city]/page'
import { generateStaticParams as generateStaticParamsCountry } from '@/app/(localized-pages)/[lang]/[country]/page'
...
async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const grouped = countryLangCityPages.reduce((acc, cur) => {
    ...
    if (lang === 'en') {
      acc[identifier].url = `https://www.heyhopping.com/en/${country}/${city}`
    } else {
      ...
      acc[identifier].alternates.languages[lang] =
        `https://www.heyhopping.com/${lang}/${country}/${city}`
    }
    return acc
  }, {} as any)
